...
Planet[] collisionPair;
ArrayList<Planet> solarPlanets = new ArrayList<Planet>();
boolean solarSystem;
...
void setup() {
  ...
  //solar system
  //13 pixels per AU of actual planet
  Planet pluto = new Planet(0.01, #dedad2, 39.0, 29.0);
}
    ...
    if (j != i) {
      if (dist(p.getPos().x, p.getPos().y, planets.get(j).getPos().x, planets.get(j).getPos().y) <
        ...
        if (p.getMass() > planets.get(j).getMass()) {
          collide(planets.get(j).getColor(), planets.get(j).getPos());
          planets.remove(j);
          angles.remove(j);
          ...
          collide(planets.get(i).getColor(), planets.get(i).getPos());
          planets.remove(i);
          angles.remove(i);
        }
      }
...
void collide(color col, PVector pos) {
  for (int i = 0; i < 20; i++) {
    fill(col);
    ellipse(pos.x, pos.y + i, 20 - i, 20 - i);
    ellipse(pos.x + 2 * i, pos.y, 20 - i, 20 - i);
    delay(3);
    fill(0);
    ellipse(pos.x, pos.y + i, 20 - i, 20 - i);
    ellipse(pos.x + 2 * i, pos.y, 20 - i, 20 - i);
